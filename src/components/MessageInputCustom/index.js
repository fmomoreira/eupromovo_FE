...
const EmojiOptions = (props) => {
  const { disabled, showEmoji, setShowEmoji, handleAddEmoji, setInputMessage } =
    props;
  const classes = useStyles();

  const [productModalOpen, setProductModalOpen] = useState();

  const handleSelectContactListModal = () => {
    // setSelectedContactId(null);
    setProductModalOpen((state) => !state);
  };
  ...
  return (
    <>
      ...
      {/* <IconButton
        aria-label="productPicker"
        component="span"
        disabled={disabled}
        onClick={(e) => setProductModalOpen((prevState) => !prevState)}
      >
      ...
      {/* {productModalOpen ? (
        <ProductListModal
          ...
          onClose={handleSelectContactListModal}
  ...
  const handleUploadAudio = async () => {
    ...
    try {
      ...
      if (blob.size < 10000) {
        setLoading(false);
        setRecording(false);
        toastError("Áudio muito curto para ser enviado.");
        return;
      }

      // Inspecionar o áudio antes de enviar
      const audioUrl = URL.createObjectURL(blob);
      const audio = new Audio(audioUrl);
      audio.play(); // Reproduz o áudio no navegador
      console.log("URL temporária do áudio:", audioUrl);

      // Exibe o áudio em uma tag <audio>
      const audioElement = document.createElement("audio");
      audioElement.src = audioUrl;
      audioElement.controls = true;
      document.body.appendChild(audioElement); // Adiciona no body para inspecionar manualmente

      // Logar informações do Blob para ver os detalhes do arquivo
