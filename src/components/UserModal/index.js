...
const UserModal = ({ open, onClose, userId, setSearchParam }) => {
  ...
  const { list: listEmpresas } = useCompanies();
  ...
  useEffect(() => {
    async function fetchData() {
      if (loggedInUser.super) {
        try {
          const response = await listEmpresas();
          console.log("LISTA EMPRESAS", response);
          setCompanies(response);
        } catch (error) {
          console.log("ERROR COMPANIE");
        }
      }
    }
    fetchData();
  ...
  //     }
  //   };
  //   console.log("USER LOGADO", loggedInUser);
  const handleSaveUser = async (values) => {
    const idEmpresa =
      companyId || values?.company?.id || loggedInUser?.companyId;
    ...
    const userData = {
      ...
      companyId: idEmpresa,
      companyIdParam: idEmpresa?.toString(),
      //   company: {
    ...
    try {
      if (userId) {
        ...
        // await api.put(`/users/${userId}/${idEmpresa}`, userData);
      } else {
                    ...
                    yes={() => (
                      ...
                      >
                        ...
                        <Field
                          ...
                          value={companyId}
                          onChange={(e) => {
                            console.log("COMPANIA", e.target.value);
                            setCompanyId(e.target.value);
                          }}
                          label="Empresa"
                        >
                          <MenuItem value={""}>&nbsp;</MenuItem>
                          {companies?.map((whatsapp) => (
